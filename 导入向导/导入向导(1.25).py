...
index = eval(input('是否更改数据类型（0：否， 1：是）：'))
while True:
    if index == 1:
        type_index = eval(input('输入要修改的列索引(输入0返回):')) - 1
        if len(column_names) > type_index >= 0:
            new_data_type = input(
                f"请输入新的数据类型 for {column_names[type_index]} (当前类型: {columns[type_index][1]}): ")
            # 构建 ALTER TABLE 语句
...
while True:
    if index == 1:
        ...
        if primary_index <= len(column_names) and primary_index >= 0:
            alter_query = f"ALTER TABLE {table_name} ADD PRIMARY KEY ({column_names[primary_index]})"
            cursor.execute(alter_query)
            break
        else:
